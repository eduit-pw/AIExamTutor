"""INF.04 mobile/web project sandbox inspector. [v1.1+ stub]"""

from __future__ import annotations

from typing import Any

from PySide6.QtWidgets import QWidget

from app.core.llm_client import LLMClient
from app.database.db_manager import DBManager
from app.workspaces.base import BaseWorkspace
from app.workspaces.factory import WorkspaceFactory


class INF04Workspace(BaseWorkspace):
    """ZIP extraction + sandbox filter. NOT IMPLEMENTED in v1.0."""

    workspace_id = "inf04"
    display_name = "INF.04 — Project Sandbox [v1.1+]"

    def __init__(self, attempt_id: int, db: DBManager, llm_client: LLMClient) -> None:
        super().__init__(attempt_id, db, llm_client)

    def build_widget(self) -> QWidget:
        raise NotImplementedError("INF04Workspace lands in v1.1. See spec §1.2 (subject #7).")

    def build_context_payload(self) -> dict[str, Any]:
        raise NotImplementedError

    def grade(self) -> dict[str, Any]:
        raise NotImplementedError


WorkspaceFactory.register(INF04Workspace.workspace_id, INF04Workspace)
